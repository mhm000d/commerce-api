...
Commerce API models the core backend of a shopping experience for a single retail business. The system supports customer authentication, product and image management, ratings, saved addresses, authenticated carts, checkout with card or cash-on-delivery payment, order state transitions, Stripe webhook processing, queued transactional email, and admin order operations.
